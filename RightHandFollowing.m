...
% Defines Function To Detect Walls (to the right) Using Direction in Which
% Robot is Facing
function [front, right] = detect_walls(x, y, direction, obstacles)
    switch direction
        case 0 
    ...
    switch direction
        case 0 
            right = is_in_obstacle(x+0.1, y, obstacles);
        case 1
            right = is_in_obstacle(x, y+0.1, obstacles);
        case 2 
            right = is_in_obstacle(x-0.1, y, obstacles);
        case 3 
            right = is_in_obstacle(x, y-0.1, obstacles);
...
while ~(abs(x - end_point(1)) < 0.1 && abs(y - end_point(2)) < 0.1)
    % Detect walls
    [front, right] = detect_walls(x, y, direction, obstacles);

...
function path = findPath(startNode, endNode, OpenNodes, ClosedNodes, Gcosts, Fcosts, Parents) %input coordinates 
    ...
    while ~isempty(OpenNodes)
        ...
        for i = 1:size(OpenNodes, 1) %taking the one with the lowest cost
            ...
            elseif  Fcosts(OpenNodes(i, 1), OpenNodes(i,2)) == Fcosts(CurrNode) & h_cost(OpenNodes(i, 1), OpenNodes(i,2)) < h_cost(CurrNode)
            CurrNode = [OpenNodes(i, 1), OpenNodes(i, 2)]
            end
        end
        ...
        for neighbor = getNeighbors(CurrNode)
            ...
            neighborPathCost = Gcosts(CurrNode)+getDistance(CurrNode, neighbor)

            if neighborPathCost<getDistance(neighbor, endNode) | ismember(neighbor, OpenNodes) == false
                Fcosts(neighbor) = neighborPathCost + getDistance(neighbor, endNode);
...
findPath([coordsToMatrix(StartNode)], [coordsToMatrix(EndNode)], OpenNodes, ClosedNodes, Gcosts, Fcosts, Parents)

...
function in_obstacle = detection(x, y, obstacles)
    ...
    for i = 1:length(obstacles)
        if obstacles{i}(round(x), round(y))
            in_obstacle = true;
            return; 
        end
    end
...
function  path = dfs(startNode, endNode, obstacles)
    path = [];
    stack = startNode;
    visited = false(101,101);
    visited(5*startNode(1)+51, 5*startNode(2)+51) = true;
    found = false;
    parent = zeros(101, 101, 2);
    while ~isempty(stack)
        currentNode = stack(end, :);
        stack(end, :) = [];
        if isequal(round(currentNode), round(endNode))
            found = true;
            break;
        end
        neighbors = find_neighbors(currentNode(1),currentNode(2),1,1,0);
        for i = 1:size(neighbors, 1)
            neighbor = neighbors(i, :);
            x_ind=5*round(neighbor(1))+51;
            y_ind=5*round(neighbor(2))+51;
            if ~detection(neighbor(1), neighbor(2), obstacles) && ~visited(x_ind, y_ind) 
                stack = [stack; neighbor]; 
                visited(x_ind, y_ind) = true;
                parent(x_ind, y_ind , :) = currentNode;
                plot(neighbor(1), neighbor(2), 'square', 'MarkerSize', 12); drawnow;
                hold on;
            end
        end
    end
